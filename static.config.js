...
export default {
    ...
    getRoutes: async () => {
        let [
            ...
            getFiles(`${NETLIFY_PATH}/roadmap`, ['.md']),
        ...
        roadmap = roadmap.map(milestone => ({
            ...milestone,
            tags: [...milestone.tags, 'roadmap']
        }))

        // add author to pages and remove pages without a path
        const allPages = [...landings, ...roadmap, ...caseStudies, ...blogPosts, ...other].filter(page => {
            if (page.path) {
        ...
        addSubpages(routes, allPages, roadmap)
